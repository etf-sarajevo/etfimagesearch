...
void libjpeg_cbir_final_stats(double cbir_featureVectorME[3][4], double cbir_featureVectorD[3][12]) {
    int i,j;
...
//            cout << "  D=" << specificBlocks[j][i];
        ...
        for (i=0; i<6; i++) {
            ...
            cbir_featureVectorD[j][i+6] = specificBlocksSquares[j][i] - specificBlocks[j][i]*specificBlocks[j][i];
	    if (cbir_featureVectorD[j][i+6] < 0) cbir_featureVectorD[j][i+6] = -cbir_featureVectorD[j][i+6];
  ...
  for (yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row;
	...
	for (yindex = 0; yindex < compptr->MCU_height; yindex++) {
	  if (cinfo->input_iMCU_row < last_iMCU_row ||
	    ...
	    for (xindex = 0; xindex < useful_width; xindex++) {
	      	  JCOEFPTR inptr;
	          inptr = (JCOEFPTR) coef->MCU_buffer[blkn+xindex];
//		printf("DC is %d\n", inptr[0]);
		libjpeg_cbir_process_data_block(inptr, ci);
	      //output_col += compptr->DCT_scaled_size;
	    }
	  }
	  blkn += compptr->MCU_width;
...
decompress_data (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  jpeg_component_info *compptr;
  inverse_DCT_method_ptr inverse_DCT;
  fprintf(stderr, "Decompress data\n");
...
decompress_smooth_data (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  int Al, pred;
//  fprintf(stderr, "Decompress smooth data\n");
